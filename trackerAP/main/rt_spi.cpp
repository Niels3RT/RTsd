...
// ****** write 8bit address/command, no data bits
void oo_RT_spi::transmit_cmd(uint8_t adr) {
	esp_err_t ret;
...
// ****** read/write 32bit from address
uint32_t oo_RT_spi::transmit32(uint8_t adr, uint32_t value, uint8_t cd) {
	esp_err_t ret;
	static spi_transaction_t trans;
	memset(&trans, 0, sizeof(spi_transaction_t));
	char rxtmp[9];
	char txtmp[9];
	memset(rxtmp, 9, sizeof(uint8_t));
	memset(txtmp, 9, sizeof(uint8_t));
	
	txtmp[0] = adr;
	txtmp[1] = (value >> 24) & 0xff;
	txtmp[2] = (value >> 16) & 0xff;
	txtmp[3] = (value >> 8) & 0xff;
	txtmp[4] = value & 0xff;

	trans.length = 40;
	trans.user = (void*)&cd;
	trans.rx_buffer = &rxtmp[0];
	trans.tx_buffer = &txtmp[0];
	trans.flags = 0x00;
	...
	// --- fetch result and return
	return((rxtmp[1]<<24) + (rxtmp[2]<<16) + (rxtmp[3]<<8) + rxtmp[4]);
}

// ****** read 64bit from spi
void oo_RT_spi::read64(char* in64) {
	esp_err_t ret;
	static spi_transaction_t trans;
	memset(&trans, 0, sizeof(spi_transaction_t));
	...
	trans.length = 64;
	trans.user = (void*)0;
	trans.rx_buffer = in64;
	trans.tx_buffer = &out64[0];
	trans.flags = 0x00;
...
void oo_RT_spi::write64(char* out64) {
	esp_err_t ret;
	static spi_transaction_t trans;
	memset(&trans, 0, sizeof(spi_transaction_t));
	...
	trans.length = 64;
	trans.user = (void*)0;
	trans.rx_buffer = &in64[0];
	trans.tx_buffer = out64;
	trans.flags = 0x00;
